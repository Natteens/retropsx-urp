    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                half3 neighbors = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, uv + float2(texel.x, 0)).rgb
                ...
                if (maskMode > 0)
                {
                    ...
                    half3 mask = phase < 1.0 ? half3(1.0, 0.75, 0.75) : (phase < 2.0 ? half3(0.75, 1.0, 0.75) : half3(0.75, 0.75, 1.0));
                    if (maskMode == 1 && fmod(input.positionCS.y, 2.0) > 1.0) mask = mask.bgr;
